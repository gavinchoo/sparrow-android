...
public class PermissionUtils {
    ...
    public static final String PERMISSION_ACCESS_COARSE_LOCATION = android.Manifest.permission.ACCESS_COARSE_LOCATION;
     ...
     */
    public static void requestPermission(final Activity activity, final int requestCode, RequestPermissionListener permissionGrant) {
        if (activity == null) {
            return;
        }

        if (requestCode < 0 || requestCode >= requestPermissions.length) {
            Log.w(TAG, "请输入正确的请求码:" + requestCode);
            return;
        }
...
//        }
        ...
        try {
            ...
            Toast.makeText(activity, "请打开应用的权限申请，不然可能无法正常的使用应用", Toast.LENGTH_SHORT)
                    .show();
    ...
    private static void requestMultiResult(Activity activity, String[] permissions, int[] grantResults, PermissionGrant permissionGrant) {
        ...
        if (notGranted.size() == 0) {
            Toast.makeText(activity, "申请成功", Toast.LENGTH_SHORT)
                    .show();
    ...
    public static void requestMultiPermissions(final Activity activity, PermissionGrant grant) {
        ...
        if (permissionsList.size() > 0) {
            ...
            showMessageOKCancel(activity, "为了正常的使用本应用，请打开相关权限",
    ...
    public static void requestPermissionsResult(final Activity activity, final int requestCode, @NonNull String[] permissions,
        ...
        if (grantResults.length == 1 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
            ...
            String[] permissionsHint = activity.getResources().getStringArray(R.array.permissions);
            openSettingActivity(activity, "SHENGQING " + permissionsHint[requestCode]);
        }
    }
    ...
    public static void applyCameraPermission(Activity context, OnOperateListener onOperateListener) {
        RxPermissions rxPermissions = new RxPermissions(context);
        rxPermissions.request(Manifest.permission.CAMERA, Manifest.permission.VIBRATE, Manifest
                .permission.WRITE_EXTERNAL_STORAGE).subscribe(aBoolean -> {
                    if (aBoolean) {
    ...
    public static void applyReadAndWritePermission(Activity context, OnOperateListener onOperateListener) {
        RxPermissions rxPermissions = new RxPermissions(context);
