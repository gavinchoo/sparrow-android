...
import com.sparrow.bundle.photo.camera.util.AngleUtil;
import com.sparrow.bundle.photo.camera.util.BitmapUtil;
import com.sparrow.bundle.photo.camera.util.CameraParamUtil;
import com.sparrow.bundle.photo.camera.util.CameraUtils;
import com.sparrow.bundle.photo.camera.util.CheckPermission;
import com.sparrow.bundle.photo.camera.util.DeviceUtil;
import com.sparrow.bundle.photo.camera.util.ScreenUtils;
...
//            cameraAngle = 270;
    ...
    public void selectCamera(boolean isFront){
        SELECTED_CAMERA = isFront ? CAMERA_FRONT_POSITION : CAMERA_POST_POSITION;
    }
    ...
    void stopRecord(boolean isShort, StopRecordCallback callback) {
        ...
        if (mediaRecorder != null) {
            ...
            try {
                ...
                mediaRecorder = null;
                mediaRecorder = new MediaRecorder();
                Log.i("CJT", "destory RuntimeException");
            } catch (Exception e) {
                e.printStackTrace();
                mediaRecorder = null;
                mediaRecorder = new MediaRecorder();
                Log.i("CJT", "destory Exception");
            } finally {
                if (mediaRecorder != null) {
